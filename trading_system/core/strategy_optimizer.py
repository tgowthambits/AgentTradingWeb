...
def test_strategy_optimizer():
    """Test function for StrategyOptimizer."""
    print("=" * 60)
    print("STRATEGY OPTIMIZER TEST")
    print("=" * 60)
    
    optimizer = StrategyOptimizer(risk_free_rate=0.05)
    
    # Create sample trades
    trades = pd.DataFrame({
        'pnl': [100, -50, 150, -30, 200, -40, 120, -60, 180, -45]
    })
    
    # Create sample equity curve
    initial_capital = 10000
    equity = initial_capital + np.cumsum(trades['pnl'].values)
    ...
    print("\n2. Sortino Ratio:")
    sortino = optimizer.calculate_sortino_ratio(returns)
    print(f"   Sortino: {sortino:.3f}")
    
    print("\n3. Maximum Drawdown:")
    dd_metrics = optimizer.calculate_max_drawdown(equity)
    print(f"   Max DD: ₹{dd_metrics['max_drawdown']:.2f}")
    print(f"   Max DD%: {dd_metrics['max_drawdown_pct']:.2f}%")
    print(f"   Max DD Duration: {dd_metrics['max_drawdown_duration']} periods")
    
    print("\n4. Win Rate:")
    win_stats = optimizer.calculate_win_rate(trades)
    print(f"   Win Rate: {win_stats['win_rate']:.1%}")
    print(f"   Avg Win: ₹{win_stats['avg_win']:.2f}")
    print(f"   Avg Loss: ₹{win_stats['avg_loss']:.2f}")
    print(f"   Win/Loss Ratio: {win_stats['avg_win_loss_ratio']:.2f}")
    
    print("\n5. Expectancy:")
    expectancy = optimizer.calculate_expectancy(trades)
    print(f"   Expectancy: ₹{expectancy:.2f} per trade")
    
    print("\n6. Profit Factor:")
    profit_factor = optimizer.calculate_profit_factor(trades)
    print(f"   Profit Factor: {profit_factor:.2f}")
    
    print("\n7. Comprehensive Metrics:")
    metrics = optimizer.get_comprehensive_metrics(trades, equity, initial_capital)
    print(f"   Total PnL: ₹{metrics['total_pnl']:.2f}")
    print(f"   Return: {metrics['total_return_pct']:.2f}%")
    print(f"   Sharpe: {metrics['sharpe_ratio']:.3f}")
    print(f"   Calmar: {metrics['calmar_ratio']:.3f}")
    print(f"   Recovery: {metrics['recovery_factor']:.2f}")
