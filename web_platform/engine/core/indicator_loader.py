...
# Resolve base paths relative to this file's location
_THIS_DIR = Path(__file__).resolve().parent          # engine/core/
_ENGINE_DIR = _THIS_DIR.parent                        # engine/
_DEFAULT_CONFIG_PATH = str(_ENGINE_DIR / "config" / "indicators_config.yaml")
_DEFAULT_INDICATORS_DIR = str(_ENGINE_DIR / "indicators")
_DEFAULT_TRADING_CONFIG_PATH = str(_ENGINE_DIR / "config" / "trading_config.yaml")
...
class IndicatorLoader:
    ...
    def __init__(self, 
                 ...
                 indicators_dir: str = _DEFAULT_INDICATORS_DIR,
                 auto_sync: bool = True,
                 trading_config_path: str = _DEFAULT_TRADING_CONFIG_PATH):
        """
    ...
    def _extract_indicator_info(self, file_path: Path) -> Dict[str, Any]:
        ...
        # engine/indicators/rsi_indicator.py -> engine.indicators.rsi_indicator
        path_parts = abs_path.parts
        
        # Find 'engine' in path
        try:
            idx = path_parts.index('engine')
            module_parts = path_parts[idx:]
            module_path = '.'.join(module_parts).replace('.py', '')
        except ValueError:
            # Fallback: use the filename
            module_path = f"engine.indicators.{file_path.stem}"
